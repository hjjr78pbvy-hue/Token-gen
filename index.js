...
const config = {
  ...
  generateUserIds: (process.env.GENERATE_USER_IDS || '').split(',').map((s) => s.trim()).filter(Boolean),
...
console.log(`  GENERATE_USER_IDS: ${config.generateUserIds.length ? config.generateUserIds.join(', ') : '(none set — optional)'}`);
...
function isAllowedToGenerate(userId) {
  return isOwner(userId) || config.generateUserIds.includes(userId);
}
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (!isAllowedToGenerate(interaction.user.id)) {
