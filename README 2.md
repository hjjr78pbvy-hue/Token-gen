...
them via `/stock` and delivered as-is via `/generate` (or the panel's Generate button).
The panel text notes that FDM.LOL refreshes the token every 5 minutes on its own end.

Every generation (slash command or panel button) DMs a `token.json` in this exact format:
...
```json
{
  "token": {
    "bearer": "...",
    "refresh_token": "...",
    "expires_at": "2026-09-04T17:49:19.000Z",
    "seconds_remaining": 148,
    "added_at": "2026-09-04T17:46:50.499Z",
    "generation_id": "GEN-1P657O"
  },
...
`generation_id` is random per delivery, and `seconds_remaining` is calculated at
generation time.
...
- **/generate**
  ...
  FDM.LOL refreshes every 5 minutes) in the channel. Clicking the button shows a short
  progress sequence (1/5 through 5/5), then builds a `token.json` (bearer token, refresh
  token, an "FDM.lol" line, expiry estimate) and DMs it with the message
  "FDM.lol json token ❤️". The file is deleted from the host right after sending.
  Anyone in `OWNER_IDS` **or** holding the `GENERATE_ROLE_ID` role can click it —
  everyone else gets rejected. Only `OWNER_IDS` can run the `/generate` command itself
  (to post the panel).

- **/remove_token**
  Owner-only. If the stocked token has passed its estimated expiry, this clears it from
  storage so `/generate` and `/check` correctly report nothing is stocked. If the token
  still looks valid, it leaves it alone and tells you how much time is left instead.
