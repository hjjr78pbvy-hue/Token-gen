...
const config = {
  ...
  generateRoleId: process.env.GENERATE_ROLE_ID || null,
...
console.log(`  GENERATE_ROLE_ID: ${config.generateRoleId || '(none set — optional)'}`);
...
function isAllowedToGenerate(interaction) {
  if (isOwner(interaction.user.id)) return true;
  if (!config.generateRoleId) return false;
  return interaction.member?.roles?.cache?.has(config.generateRoleId) ?? false;
}
...
async function generateAndDeliver(user) {
  ...
  const expiresMs = store.expires_at || 0;
  ...
  const payload = {
    token: {
      bearer: store.bearer_token,
      refresh_token: store.refresh_token,
      expires_at: expiresMs ? new Date(expiresMs).toISOString() : null,
      seconds_remaining: secondsRemaining,
      added_at: store.last_refreshed
        ? new Date(store.last_refreshed).toISOString()
        : new Date().toISOString(),
      generation_id: generationId,
    },
    message: 'FDM.LOL Auto-Delivery (every 5 min)',
    credits: 'Faxx',
  ...
  try {
    ...
    await user.send({ content: 'FDM.lol json token ❤️', files: [attachment] });
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Remove the stocked token if it is expired or invalid'),
  new SlashCommandBuilder()
    .setName('check')
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (!isAllowedToGenerate(interaction)) {
    ...
    const steps = [
      '1/5 Verifying account...',
      '2/5 Checking token stock...',
      '3/5 Preparing file...',
      '4/5 Building JSON...',
      '5/5 Sending to DMs...',
    ];
  ...
  if (interaction.isModalSubmit()) {
    ...
    await interaction.reply({ content: 'Token stocked.', ephemeral: true });
    return;
  }
  ...
  if (interaction.commandName === 'remove_token') {
    const store = loadStore();

    if (!store.bearer_token) {
      await interaction.reply({ content: 'Nothing stocked — there is no token to remove.', ephemeral: true });
      return;
    }

    const msLeft = (store.expires_at || 0) - Date.now();
    const isExpired = msLeft <= 0;

    if (!isExpired) {
      const minutesLeft = Math.floor(msLeft / 60000);
      await interaction.reply({
        content: `Stocked token still looks valid (about ${minutesLeft} minute(s) left) — not removing it. Use /stock to overwrite it if you want to replace it anyway.`,
        ephemeral: true,
      });
      return;
    }

    saveStore({ bearer_token: null, refresh_token: null, expires_at: null, last_refreshed: null });
    await interaction.reply({ content: 'Removed the expired/invalid token. Use /stock to add a new one.', ephemeral: true });
  }
});
