...
A private Discord bot for you and your trusted friends to manage a single bearer/refresh
token pair against your own API. Not meant for public servers or distributing tokens to
strangers — commands are locked to the Discord user IDs you set in `OWNER_IDS`.
...
- **/stock** `bearer_token` `refresh_token` `refresh_url` `[expires_in]`
  Saves your token pair (in `tokenStore.json` on the host) and starts the auto-refresh
  cycle.

- **/generate**
  Posts a panel (an embed with a "Generate" button) in the channel. Clicking the button
  builds a `token.json` (bearer token, refresh token, expiry) and DMs it to whoever
  clicked. The file is deleted from the host right after sending. Only users in
  `OWNER_IDS` can click it — anyone else gets rejected.
...
Every 5 minutes, the bot POSTs `{ "refresh_token": "..." }` as JSON to the `refresh_url`
you gave it in `/stock`. It expects a JSON response containing a new `bearer_token` (or
`access_token`), a `refresh_token`, and optionally `expires_in` (seconds). The result
overwrites `tokenStore.json`.
...
- `tokenStore.json` holds a live bearer/refresh token in plaintext on Railway's disk.
  **Railway's filesystem is ephemeral** — it resets on every redeploy, so you'll need to
  re-run `/stock` after any redeploy. If you want the token to survive redeploys, ask and
  I can wire it up to store in a Railway environment variable or a small database instead.
