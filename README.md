...
There is no background auto-refresh — tokens are stored exactly as you enter them via
`/stock` and delivered as-is via `/generate`.
...
4. **Set environment variables** in Railway: open the deployed service → Variables tab
   ...
   - `DEFAULT_EXPIRY_SECONDS` — optional, defaults to 3600 (used only to estimate a
     countdown for `/check` since there's no live refresh to confirm real expiry)
...
- **/generate**
  ...
  Clicking the button builds a `token.json` (bearer token, refresh token, expiry
  estimate) and DMs it to whoever clicked. The file is deleted from the host right after
  sending. Only users in `OWNER_IDS` can click it — anyone else gets rejected.
...
- **/check**
  ...
  and when it was last stocked. Since there's no live refresh, this is an estimate, not
  a server-confirmed check.
