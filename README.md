...
4. **Set environment variables** in Railway: open the deployed service → Variables tab
   ...
   - `DEFAULT_EXPIRY_SECONDS` — optional, defaults to 3600 (used only to estimate a
     countdown for `/check`)
   - `GENERATE_USER_IDS` — optional, comma-separated Discord user IDs who can click the
     **Generate** button on the panel but cannot run `/stock`, `/generate`, or `/check`
     themselves
...
- **/generate**
  ...
  **or** `GENERATE_USER_IDS` can click it — everyone else gets rejected. Only
  `OWNER_IDS` can run the `/generate` command itself (to post the panel).
