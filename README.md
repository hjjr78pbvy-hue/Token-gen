...
4. **Set environment variables** in Railway: open the deployed service → Variables tab
   ...
   - `REFRESH_URL` — your API's token refresh endpoint (used for every auto-refresh)
...
- **/stock**
  ...
  and saves them (in `tokenStore.json` on the host) once submitted. The refresh endpoint
  is fixed via the `REFRESH_URL` environment variable, so you don't enter it each time.
